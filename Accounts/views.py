...
def send_verification_email(user, request):
    ...
    verification_link = request.build_absolute_uri(reverse('accounts:verify_email', kwargs={'uidb64': uid, 'token': token}))
...
def VerifyOTP(request, user_id):
    if request.method == 'POST':
        ...
        if stored_otp and entered_otp == stored_otp:
            ...
            return redirect('accounts:PasswordReset', user_id=user_id)
        else:
