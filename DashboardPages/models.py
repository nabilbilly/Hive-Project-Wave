...
class UserJobProfile(models.Model):
    ...
    interests = models.TextField(blank=True)  # Store as comma-separated values
    ...
    work_schedule = models.CharField(max_length=50, choices=[
        ('full-time', 'Full Time'),
        ('part-time', 'Part Time'),
        ('flexible', 'Flexible')
    ])
    email_notifications = models.BooleanField(default=True)
    skills = models.ManyToManyField('Accounts.Skill', related_name='user_profiles')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Job Profile for {self.user.username}"
...
class UserIntroduction(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    interests = models.JSONField(default=list)  # Store selected interests
    expected_salary = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    salary_type = models.CharField(max_length=10, choices=[('hourly', 'Hourly'), ('monthly', 'Monthly'), ('yearly', 'Yearly')], default='yearly')
    years_experience = models.IntegerField(null=True, blank=True)
    education_level = models.CharField(max_length=50, null=True, blank=True)
    english_level = models.CharField(max_length=20, null=True, blank=True)
    ...
    team_setup = models.CharField(max_length=50, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"{self.user.username}'s Introduction Data"
